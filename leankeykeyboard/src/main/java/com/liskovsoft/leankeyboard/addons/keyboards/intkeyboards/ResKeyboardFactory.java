...
public class ResKeyboardFactory implements KeyboardFactory {
    ...
    public List<? extends KeyboardBuilder> getAllAvailableKeyboards(Context context) {
        ...
        for (final KeyboardInfo info : infos) {
            if (info.isEnabled()) {
                result.add(new ResKeyboardBuilder(context, info));
            }
        }
        ...
        if (result.isEmpty()) {
            ...
            result.add(new ResKeyboardBuilder(mContext, defaultKbd));
    ...
    public class ResKeyboardBuilder implements KeyboardBuilder {
        ...
        private final KeyboardInfo info;
        private final String langCode;

        public ResKeyboardBuilder(Context ctx, KeyboardInfo info) {
            this.ctx = ctx;
            this.info = info;
            this.langCode = info.getLangCode();
        }
        ...
        public Keyboard createAbcKeyboard() {
            ...
            int kbResId = ctx.getResources()
                    .getIdentifier(prefix + langCode, "xml", ctx.getPackageName());
            Keyboard keyboard = new Keyboard(ctx, kbResId);
        ...
        public Keyboard createSymKeyboard() {
            Keyboard keyboard = new Keyboard(ctx, R.xml.sym_en_us);
            return localizeKeys(keyboard, info);
        }

        @Override
        public Keyboard createNumKeyboard() {
            return new Keyboard(ctx, R.xml.number);
        }
    }
